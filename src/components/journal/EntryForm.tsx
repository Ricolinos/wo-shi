...
import { saveEntry, updateEntry } from "@/lib/actions/entry.actions"
import type { getJournalEntry } from "@/lib/actions/entry.actions"
...
/** Entrada completa devuelta por `getJournalEntry` — usada para prellenar el modo edición. */
export type JournalEntryDetail = NonNullable<Awaited<ReturnType<typeof getJournalEntry>>>

type EntryFormProps =
  | { mode: "create"; initialEntry?: undefined }
  | { mode: "edit"; initialEntry: JournalEntryDetail }

function pad(n: number) {
  return String(n).padStart(2, "0")
}

/** Convierte la entrada ya guardada (fecha/hora como Date) al shape plano que edita el formulario. */
function draftFromEntry(entry: JournalEntryDetail): EntryDraft {
...
export function EntryForm({ mode, initialEntry }: EntryFormProps) {
  const router = useRouter()
  const { addToast } = useToast()
  const [draft, setDraft] = useState<EntryDraft>(() => (mode === "edit" ? draftFromEntry(initialEntry) : EMPTY_DRAFT))
  ...
  // Vínculos ya asociados a la entrada en modo edición — solo lectura, ya que
  // updateEntry ignora a propósito persons/emotions/ideas del draft.
  const existingPersons = isEdit ? initialEntry.entryBonds.filter(eb => eb.bond.type === "PERSON") : []
  const existingEmotions = isEdit ? initialEntry.entryBonds.filter(eb => eb.bond.type === "EMOTION") : []
  const existingIdeas = isEdit ? initialEntry.entryBonds.filter(eb => eb.bond.type === "IDEA" || eb.bond.type === "BELIEF") : []
  const existingMedia = isEdit ? initialEntry.media : []
  ...
  async function handleSave(asDraft = false) {
    ...
    try {
      ...
      const res = isEdit ? await updateEntry(initialEntry.id, fd) : await saveEntry(fd)
      if (!res.ok) { setSaveError(res.error); return }
      router.push(isEdit ? `/journal/${res.entryId}` : asDraft ? "/journal" : `/journal/${res.entryId}`)
    } catch {
      ...
      <Column fillWidth maxWidth="m" paddingY="24" paddingX="16" gap="16" style={{ margin: "0 auto" }}>
        <Row horizontal="between" vertical="center">
          <Row gap="12" vertical="center">
            ...
            <Heading variant="display-strong-xs">{isEdit ? "Editar entrada" : "Nueva entrada"}</Heading>
          </Row>
          <Row gap="8">
            {!isEdit && <Button variant="secondary" onClick={() => handleSave(true)} disabled={isSaving}>Guardar borrador</Button>}
            <Button variant="primary" onClick={() => handleSave(false)} disabled={isSaving || !draft.body.trim()} loading={isSaving}>
        ...
        <Column gap="8" padding="16" radius="l" border="neutral-alpha-weak" background="surface">
          ...
          {isEdit && <Text variant="body-default-s" onBackground="neutral-weak">Los vínculos no se pueden editar por ahora.</Text>}
          <Row gap="8" wrap>
            {isEdit
              ? existingPersons.map(eb => (
            ...
            {!isEdit && <Button variant="secondary" onClick={() => setModal("person")}>+ Agregar persona</Button>}
          </Row>
        </Column>
        ...
        <Column gap="8" padding="16" radius="l" border="neutral-alpha-weak" background="surface">
          ...
          <Row gap="8" wrap>
            ...
            {!isEdit && <Button variant="secondary" onClick={() => setModal("emotion")}>+ Agregar emoción</Button>}
          </Row>
        </Column>
        ...
        <Column gap="8" padding="16" radius="l" border="neutral-alpha-weak" background="surface">
          ...
          <Row gap="8" wrap>
            ...
            {!isEdit && <Button variant="secondary" onClick={() => setModal("idea")}>+ Agregar idea o creencia</Button>}
          </Row>
        </Column>
        ...
        <Column gap="8" padding="16" radius="l" border="neutral-alpha-weak" background="surface">
          ...
          {existingMedia.length > 0 && (
            <Row gap="8" wrap>
              {existingMedia.map(m => (
                <Row key={m.id} width="64" height="64" radius="m" border="neutral-alpha-weak" style={{ overflow: "hidden" }}>
                  {m.type === "IMAGE"
